...
/*
 ...
 - minimize screen yaw-estate usage
...
!- color: the color helpers/typing is a mess and needs sorting out.
 ...
 - slider: add dragging-based widgets to edit values with mouse (on 2 axises), saving screen yaw-estate.
...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding)
{
    ...
    if (border && (window->Flags & ImGuiWindowFlags_ShowBorders))
    {
        ...
        window->DrawList->AddRect(p_min + ImVec2(1, 1), p_max - ImVec2(1, 1), GetColorU32(ImGuiCol_Border), rounding);
    }
}
...
bool ImGui::ColorPicker(const char* label, NSDK::Color* color)
{
	...
	NSDK::Color colors[] = { NSDK::Color(255, 0, 0),
		NSDK::Color(255, 255, 0),
		NSDK::Color(0, 255, 0),
		NSDK::Color(0, 255, 255),
		NSDK::Color(0, 0, 255),
		NSDK::Color(255, 0, 255),
		NSDK::Color(255, 0, 0) };
	...
	for (int i = 0; i < 6; ++i)
	{
		draw_list->AddRectFilledMultiColor(
			...
			ImGui::ColorConvertFloat4ToU32(ImVec4(colors[i].r(), colors[i].g(), colors[i].b(), colors[i].a())),
			ImGui::ColorConvertFloat4ToU32(ImVec4(colors[i].r(), colors[i].g(), colors[i].b(), colors[i].a())),
			ImGui::ColorConvertFloat4ToU32(ImVec4(colors[i + 1].r(), colors[i + 1].g(), colors[i + 1].b(), colors[i + 1].a())),
			ImGui::ColorConvertFloat4ToU32(ImVec4(colors[i + 1].r(), colors[i + 1].g(), colors[i + 1].b(), colors[i + 1].a())) );
	}

	float hue, saturation, value;
	ImGui::ColorConvertRGBtoHSV(
		color->r(), color->g(), color->b(), hue, saturation, value);
	...
	ImColor hsv = ImColor::HSV(hue, saturation, value);
	*color = NSDK::Color(hsv.Value.x, hsv.Value.y, hsv.Value.z, hsv.Value.w );
	return value_changed;
}

bool ImGui::ColorPicker(const char* label, ImColor* color)
{
	static const float HUE_PICKER_WIDTH = 20.0f;
	static const float CROSSHAIR_SIZE = 7.0f;
	static const ImVec2 SV_PICKER_SIZE = ImVec2(200, 200);
